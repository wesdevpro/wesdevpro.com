...
Blue-and-teal, reading-first, **precise and engineered**. Flat surfaces, hairline borders, tight radii, three type voices, and exactly two signatures.
...
Never add a third signature. If a design needs more interest, tighten structure instead.
...
- **Three type voices.** Space Grotesk = all headings (600/700, tracking `-0.02em`). Inter = body, nav, buttons, controls (line-height 1.68, prose 1.72, `text-wrap: pretty`). Inconsolata = eyebrows, labels, tags, metadata, code. All-Inter is the #1 default tell — never ship it.
- **80/20 color.** Blue carries ~80% (the core: integrations + web). Teal is the ~20% craft signal — signatures, OSS callouts, hover emphasis only. General-purpose teal kills the signal.
...
**Out of scope unless asked:** new pages or sections, new dependencies, wholesale content rewrites, framework/build changes. Fix obviously off-brand microcopy in passing; suggest anything bigger rather than doing it.
